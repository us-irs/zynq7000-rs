...
- The [`examples`](./firmware/examples)
  ...
  [`embassy`](https://github.com/embassy-rs/embassy) native Rust asynchronous run-time and SMP
  applications running both cores.
...
integration by using the [`probe-rs`](https://probe.rs) library to talk to the debug probe directly.
In a single invocation it:

- Connects to a JTAG debug probe and resets/halts the core.
- Executes the PS7 (PLL/clock/DDR/post-config) init sequence, read either directly from a raw
  `ps7_init.tcl` script or from a RON file as generated by
  [`z7-ps7init-extract`](./host/z7-ps7init-extract). This includes the post-config step that
  brings the PL out of its power-on reset state.
...
- The Z7 register initialization sequence, as either a raw `ps7_init.tcl` script or a RON file
  generated from one using the [`z7-ps7init-extract`](./host/z7-ps7init-extract) tool. We provide
  a Zedboard compatible RON file at `scripts/zedboard_init_regs.ron`.
